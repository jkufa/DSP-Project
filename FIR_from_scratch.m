...
%formatting
title('HP Filtered Power Spectrum for tyler_orginal.ogg');
xlabel('Frequency (Hz)');
ylabel('Power');
xlim([0 3000])
ylim([0 60])
...
audiowrite('tyler_scratchfilter_highpass.wav',xtrafil,Fs)
...
plot(f(1:floor(m1/2)), power2(1:floor(m1/2)));
%formatting
title('LP Filtered Power Spectrum for tyler_orginal.ogg');
xlabel('Frequency (Hz)');
ylabel('Power');
xlim([0 3000])
ylim([0 60])
...
player = audioplayer(xtrafil, Fs);
play(player,[1 (get(player, 'SampleRate')*3)]);
audiowrite('tyler_scratchfilter_lowpass.wav',xtrafil,Fs)
